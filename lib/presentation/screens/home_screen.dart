    ...
    ref.listen<AsyncValue<UpdateInfo?>>(
      ...
      (previous, next) {
        if (next.hasValue && next.value != null) {
          // If update is available, we could automatically show the bottom sheet,
          // but the banner is enough. If we wanted to pop it up:
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // ── Кнопки управления ──────────────────────────────────────────
          GlassIconBtn(icon: Icons.terminal_rounded, onTap: onLogTap),
          const SizedBox(width: 8),
          GlassIconBtn(icon: Icons.tune_rounded, onTap: onSubTap),
          const SizedBox(width: 8),
          GlassIconBtn(
